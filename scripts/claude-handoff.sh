...
case "$mode" in
  spark)
    ...
    exec muse exec --model "$MUSE_SPARK_MODEL" --trust-workspace --workspace "$ROOT" --max-model-steps "${MUSE_MAX_STEPS:-30}" --reasoning-effort "${MUSE_EFFORT:-high}" --prompt-file "$prompt_file" ${images[@]+"${images[@]}"}
    ;;
  glm|flash)
    if [[ "$mode" == "glm" ]]; then
