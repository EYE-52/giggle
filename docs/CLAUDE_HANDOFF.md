...
- **Avatars refresh (commit `4c85e3a`, deployed):** the 16 legacy motif avatars no longer render anywhere on web. The picker offers 12 character presets plus Upload. Legacy ids stay valid data but render as matching characters (`legacyCharacterFor`). People without an avatar get `seededCharacterFor(userId)`. Muse Spark did the "lively" design pass and review: bolder palette additions (shirt/accent/accessory only; the skin and hair palettes must stay equal to `server/src/utils/avatarMatch.js`), varied expressions, and animation for avatars ≥72px. Note: the Vibe Pack perk in billing.ts still advertises "extra profile avatars" but no longer unlocks anything. The mobile app still draws the legacy motifs.
