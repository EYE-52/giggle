...
1. **Uploaded squad covers live in their own collection, behind `server/src/services/coverStorage.js`.** Squad documents carry only a short reference (`coverImage: "upload:<squadId>/<hash>"`), so squad lists, lobby polling and matchmaking never load image bytes. The bytes (up to ~1.5 MB) sit in the `squadcovers` collection, one row per image. Clients get `GET /api/covers/:squadId/:hash`, served with an immutable cache header; the hash is a content hash of the image and makes the URL unguessable. Replacing a cover deletes the old image, and deleting a squad deletes all of its images. Squads saved before this change hold a base64 data URL and keep working. The server migrates them automatically about 15 seconds after it starts: the job is batched and idempotent, runs on one instance only (Redis lease), and is skipped with `COVER_BACKFILL_ON_START=false`. `npm run migrate:covers` in `server/` (with `-- --dry-run` first) runs the same migration by hand. To move images to object storage (S3/R2), write a backend for the four functions documented at the top of `coverStorage.js`, select it with `COVER_STORAGE=s3`, and copy the existing `squadcovers` rows across. Uploads still arrive as data URLs in JSON, so the 3 MB body limit in `server.js` stays until clients upload directly to storage.
