...
export function Logomark({ size = 32, animated = false }: { size?: number; glow?: boolean; animated?: boolean }) {
  ...
  return <svg className={`gg-logomark logo ${drawing ? 'gg-logo-draw' : ''}`} width={size} height={size} viewBox={logoViewBox} fill="none" aria-hidden="true" style={{ color: 'var(--accent, #ba4b33)' }}>
