...
4. Check `/health` for API `UP`, database `connected`, and Redis `connected`. Run the signed-in production smoke test: `pnpm --filter @giggle/desktop test:prod-smoke`. It signs in the synthetic account `prod-smoke@e2e.gigglemeet.test` through the existing server-to-server auth exchange, checks home, friends, wallet, profile and the avatar, then creates a squad and leaves it again. It needs Railway's `AUTH_EXCHANGE_SECRET` in `GIGGLE_AUTH_EXCHANGE_SECRET` or in the gitignored `apps/desktop/.env.prod-smoke` (`GIGGLE_AUTH_EXCHANGE_SECRET=...`), and skips without it. Open the public website, sign in with Google, and confirm access to `/home`. For a new account, test the date-of-birth form with an authorized adult test identity; do not overwrite a real user's saved birth date.
